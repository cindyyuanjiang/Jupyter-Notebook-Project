...
function printDependenciesNoTypeInf(cell_to_lines, line_dep, line_cfg_dep, line_cfg_dep_btwn_cells, 
                           printMode, dict, name, res_color_map, sources, sinks, cell_counts) {
    let res = "";
    var dep_count = 0;
    var cfg_count = 0;
    ...
    res += cfg_btwn_cnt;
    var new_name = name.substring(0, name.lastIndexOf('.'));
    fs.writeFile(new_name + '_new_labels_no_type_inf.txt', res, function (err) {
      if (err) throw err;
    });
}
...
module.exports = {
    ...
    calculateCellsNoTypeInf: function(name, printMode) {
        const programSrc = fs.readFileSync(name).toString();
        const programJson = JSON.parse(programSrc);
        let jsonName = name.substring(0, name.lastIndexOf(".")) + "_no_comments.json";
        const jsonSrc = fs.readFileSync(jsonName).toString();
        ...
        printDependenciesNoTypeInf(cell_to_lines, line_dep, line_cfg_dep, line_cfg_dep_btwn_cells, printMode, 
                          dict, name, res_color_map, sources, sinks, Array.from(all_line_nos));
    }
}