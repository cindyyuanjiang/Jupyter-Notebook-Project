...
// hyperparameter-searching models like "ParameterSampler" are temporarily labeled as supervised
...
function outputDependenciesResult(cell_to_lines, line_dep, line_cfg_dep, line_cfg_dep_btwn_cells, 
                           printMode, dict, name, res_color_map, sources, sinks, cell_counts) {
    let res = "";
    var dep_count = 0;
    var cfg_count = 0;
...
module.exports = {
        ...
        for (let flow of flows.items) {
            if (py.printNode(flow.toNode) != undefined && py.printNode(flow.toNode).includes('.fit')) {
                ...
                if ((!(toNodeLineNo in map_from_line_to_label)) && fromNodeLineNo in map_from_line_to_label && map_from_line_to_label[fromNodeLineNo] == 'training') {
                    ...
                    // if (sk_training_models.has(trainingModel)) {
                    //     if (trainingModel in model_counter) {
                    //         model_counter[trainingModel] += 1;
                    //     } else {
                    //         model_counter[trainingModel] = 1;
                    //     }
                    // } else if ((jsonFile.hasOwnProperty(trainingModel)) && sk_training_models.has(jsonFile[trainingModel][1])) {
                    //     if (jsonFile[trainingModel][1] in model_counter) {
                    //         model_counter[jsonFile[trainingModel][1]] += 1;
                    //     } else {
                    //         model_counter[jsonFile[trainingModel][1]] = 1;
                    //     }
                    // }
    ...
    calculateCellsForTesting: function(name, printMode) {
        const programSrc = fs.readFileSync(name).toString();
        const programJson = JSON.parse(programSrc);
        // let jsonName = name.substring(0, name.lastIndexOf(".")) + "_no_comments.json";
        // const jsonSrc = fs.readFileSync(jsonName).toString();
        ...
        try {
            ...
            const jsonSrc = fs.readFileSync(jsonName).toString();
            jsonFile = JSON.parse(jsonSrc);
            // console.log(jsonFile);
        } catch (err) {
            // const jsonFile = new Object();
        }
        let model_counter = new Object();
        //dict is a dictionary pointing from execution_count to the corresponding cell 
        let dict = new Object();
        let text = "";
        let cells = [];
            ...
            if (cell.cell_type === 'code') {
                ...
                for (let line of cell.source) {
                    if (line[0] != '#' && line[0] != '%' && line[0] != '!') {
                        ...
                        if (!line.includes("import ") && flag && line.includes(plt.toString())) {
                            map_from_line_to_label[currentLineNo + 1] = "plotting";
                        ...
                        if ((currentLineNo + 1) in map_from_line_to_label) {
                            if (map_from_line_to_label[(currentLineNo + 1)] == "data collection") {
                                res_color_map += ((currentLineNo + 1) + '->' + 'red' + '\n');
                            } else if (map_from_line_to_label[(currentLineNo + 1)] == "data processing") {
                                res_color_map += ((currentLineNo + 1) + '->' + 'yellow' + '\n');
                            } else if (map_from_line_to_label[(currentLineNo + 1)] == 'feature engineering') {
                                res_color_map += ((currentLineNo + 1) + '->' + 'green' + '\n');
                            } else if (map_from_line_to_label[(currentLineNo + 1)] == 'training') {
                                res_color_map += ((currentLineNo + 1) + '->' + 'purple' + '\n');
                            } else if(map_from_line_to_label[(currentLineNo + 1)] == 'evaluation') {
                                res_color_map += ((currentLineNo + 1) + '->' + 'orange' + '\n');
                            } else if(map_from_line_to_label[(currentLineNo + 1)] == 'plotting') {
                                res_color_map += ((currentLineNo + 1) + '->' + 'lightblue' + '\n');
                            }
                        }
            ...
            for (let succ_block of succ) {
                ...
                if (succ_stmts[0] == undefined) {
                    ...
                    for (let subSuccBlock of subSucc) {
                        ...
                        let subToLineNo = subSuccStmts[0].location.first_line;
                        if (!line_cfg_dep[from_line_no].includes(subToLineNo)) {
                            line_cfg_dep[from_line_no].push(subToLineNo);
                        }
                        all_line_nos.add(subToLineNo);
                    }
                ...
                if (from_line_no in line_cfg_dep && !line_cfg_dep[from_line_no].includes(to_line_no)) {
                    line_cfg_dep[from_line_no].push(to_line_no);
                    // console.log("from" + from_line_no);
                    // console.log(line_cfg_dep[from_line_no]);
                }
            }
        }
        ...
        for (let flow of flows.items) {
            if (py.printNode(flow.toNode) != undefined && py.printNode(flow.toNode).includes('.fit')) {
                ...
                if ((!(toNodeLineNo in map_from_line_to_label)) && fromNodeLineNo in map_from_line_to_label && map_from_line_to_label[fromNodeLineNo] == 'training') {
                    let trainingModel = find_model(py.printNode(flow.toNode));
                    console.log("605!!");
                    console.log(py.printNode(flow.toNode));
                    console.log(trainingModel);
                    console.log(jsonFile);
                    if ((jsonFile.hasOwnProperty(trainingModel)) && sk_training_models.has(jsonFile[trainingModel][1])) {
                        console.log("609!!");
                        console.log(jsonFile[trainingModel][1]);
                        console.log(toNodeLineNo);
                        map_from_line_to_label[toNodeLineNo] = 'training';
                        res_color_map += (toNodeLineNo + '->' + 'purple' + '\n');
                    }
                    if (sk_training_models.has(trainingModel)) {
                ...
                if (fromNodeLineNo in map_from_line_to_label && map_from_line_to_label[fromNodeLineNo] == 'feature engineering' && (!(toNodeLineNo in map_from_line_to_label))) {
                    map_from_line_to_label[toNodeLineNo] = 'feature engineering';
                    res_color_map += (toNodeLineNo + '->' + 'green' + '\n');
                }
            } else if (py.printNode(flow.toNode) != undefined && py.printNode(flow.toNode).includes('.predict')) {
                let toNodeLineNo = flow.toNode.location.first_line;
                let fromNodeLineNo = flow.fromNode.location.first_line;
                if (fromNodeLineNo in map_from_line_to_label && map_from_line_to_label[fromNodeLineNo] == 'training' && (!(toNodeLineNo in map_from_line_to_label))) {
                    map_from_line_to_label[toNodeLineNo] = 'evaluation';
        ...
        for (let flow of flows.items) {
            ...
            all_line_nos.add(fromNodeLineNo);
            all_line_nos.add(toNodeLineNo);
            // console.log(fromNodeLineNo + ': ' + py.printNode(flow.fromNode) + '-->');
            // console.log(toNodeLineNo + ': ' + py.printNode(flow.toNode));
            // console.log('\n\n');
            if (py.printNode(flow.fromNode) != undefined && py.printNode(flow.fromNode).includes('import ')) {
                continue;
            }
            ...
            all_defs.add(fromNodeLineNo);
            // console.log("all_defs push" + fromNodeLineNo + '\n');
            all_uses.add(toNodeLineNo);
            // console.log("all_uses push" + toNodeLineNo + '\n');
        }
        // console.log("!!!!!!!!" + all_defs);
        // console.log(all_uses);
        let sources = [];
        let sinks = [];
        for (let line of all_line_nos) {
            if (!all_uses.has(line)) {
                sources.push(line);
                // console.log("source push" + line + '\n');
            }
            if (!all_defs.has(line)) {
                sinks.push(line);
                // console.log("sink push" + line + '\n');
            }
        }
        let final_res = outputDependenciesResult(cell_to_lines, line_dep, line_cfg_dep, line_cfg_dep_btwn_cells, printMode, 
                          dict, name, res_color_map, sources, sinks, Array.from(all_line_nos));
        return [final_res, model_counter];
        // return final_res;
    }
}